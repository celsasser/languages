...
A library package looks the same but with `src/` instead. A package can contain both a `main.rs` and a `lib.rs`, producing a binary and a library from the same source tree.
...
#### Unit Type
In Rust, `()` is called the **unit type** (and also the **unit value**). It is essentially Rust's version of `void` from languages like C, C++, or Java, but with a key difference: it is an actual value that occupies zero bytes of memory. You will frequently see it used in `Okay(...)` statements: `Okay(())`. Looks a little weird doesn't it? It looks like we are calling a function without specifying the function name. Here is the breakdown of `Ok(())`:
...
Instead, Rust uses an enum called Option<T> which we saw up above.
...
When `s` goes out of scope, there are no remaining pointers to the memory that `s` held. This is an ideal point at which we may return `s`'s memory to the allocator. When a variable goes out of scope, Rust calls a special function for us: `drop`. It’s where the author of `String` *can* put the code to return the memory to the allocator. Rust calls `drop` automatically at the closing curly bracket. This is sort of analagous to what happens in C++ when `delete` is called on an pointer: its destructor[s] is called and the memory is freed. The difference here is that it seems that its up to the author to release the memory. My memory of C++ destructors is foggy; I think that by the time a destructor was called, his fate was sealed: he's a goner.
