...
###### Sorting
`Vec<T>` provides two sorting methods. The right choice depends on whether your element type implements `Ord` and whether you need stability.
...
Floating-point types (`f32`, `f64`) do not implement `Ord` because `NaN` breaks total ordering. Use `.sort_by()` with `.partial_cmp()` and handle the `None` case:

```rust
let mut floats = vec![3.1, 0.5, 2.7, 1.4];
...
[clap](https://docs.rs/clap/latest/clap/){target="_blank" rel="noopener noreferrer"} (Command Line Argument Parser) turns a Rust struct into a fully-featured CLI: `--help`, `--version`, validation, subcommands, and shell completions all come for free. You describe what your program accepts; clap handles the rest.
