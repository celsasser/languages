...
## Option\<T\>
I use this guy very frequently, and I very frequently forget how to use him. Hence his high presence on this Rusty totem pole.
...
The central question with `Option<T>` is always: how do I get it out of there? It all depends on what your goal is.
...
#### `if let` — handle only `Some`, ignore `None`
...
#### `unwrap` and `expect` — crash on `None`
...
#### `.filter(predicate)` — keep `Some` only if condition holds
