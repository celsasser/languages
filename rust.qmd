...
Cargo manages a well-known list of [environment variables](https://doc.rust-lang.org/cargo/reference/environment-variables.html){target="_blank"}. They fall into to major categories:
...
##### `env` the Function
Rust includes the [`env`](https://doc.rust-lang.org/stable/std/env/){target="_blank"} module as part of the standard library. It includes functions to read and write environment variables.
...
Cargo's environment variable list is very long, so we've picked out some fan favorites as a sampler platter. See the official documentation (which I think is): [The Cargo Book](https://doc.rust-lang.org/cargo/reference/environment-variables.html){target="_blank"}.
